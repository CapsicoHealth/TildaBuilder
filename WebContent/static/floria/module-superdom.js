...
export var SuperDOM = {
   ...
   getAncestorNode: function(e, nodeName)
    ...
    nextSiblingNode: function(elementId)
     {
       var e = SuperDOM.getElement(elementId).nextSibling;
    ...
    insertAfter: function(target, newElement)
     {
       var e = SuperDOM.getElement(target);
       if (e != null)
...
// console.log("Starting waiter "+this.waiterName+"."+this.elementId);
     ...
     addEvent: function(elementId, eventName, handlerFunc, delayedMillis)
      {
        ...
        if (delayedMillis == null)
          e.addEventListener(eventName, function(event) { return handlerFunc(e, event, event.target); });
        else
          e.addEventListener(eventName, function(event) { SuperDOM.DelayedEvent.register(elementId, delayedMillis, function() { handlerFunc(e, event, event.target); }); });
      },
     ...
     setupTooltip: function(elementId, eventName, tooltipTextClassName, ajaxUrlFunc, contentsFunc)
      {
        SuperDOM.addEvent(elementId, eventName, function(e, event, target) {
     ...
     filterRows: function(tableId, inputId)
      {
        var t = SuperDOM.getElement(tableId);
        if (t == null || t.rows == null || t.rows.length == 0)
         return;
        var e = SuperDOM.getElement(inputId);
        if (e == null)
          return;
     ...
     getFormElementValue: function (formElement, elementName)
      {
        if (formElement == null)
         return null;
        var f = SuperDOM.getElement(formElement);
        if (formElement == null)
          return console.warn("SuperDOM.getFormElementValue: Cannot find form '"+formElement+".");
        var e = f[elementName];
        if (e == null)
          return console.warn("SuperDOM.getFormElementValue: Cannot find element '"+elementName+"' in Form "+formElement+".");
        return SuperDOM.getElementValue(e);
      },
     getElementValue: function (e)
      {
        e = SuperDOM.getElement(e);
        if (e == null)
          return console.warn("SuperDOM.getElementValue was passed a null element");
     ...
     {
       var parentElement = SuperDOM.getElement(parentId, "Cannot find Element with id='"+parentId+"'");
...
// "+document.documentElement.clientWidth+";");
        return SuperDOM.isElementHidden(el) == false
     ...
     showImgFullScreen: function(divId, imgSrc, closeFunc, waitingAnimSrc)
      {
        var e = SuperDOM.getElement(divId);
        if (e == null)
         return;
     ...
     resizeImages: function(divId, imgBoxSize)
      {
        ...
        if (stillLoading == true)
         {
           setTimeout(function() { SuperDOM.resizeImages(divId, imgBoxSize); }, 20);
           return;
         }
      },
     ...
     showImgGallery: function(divId, imgBoxSize, imgSrcs)
      {
        ...
        if (imgSrcs == null || imgSrcs.length == 0)
         return SuperDOM.setInnerHTML(e, "<BR>No images found");
        ...
        for (var i = 0; i < imgSrcs.length; ++i)
         {
           if (SuperDOM.isObject(imgSrcs[i]) == true)
             {
               if (i != 0)
                str=='</TR>';
        ...
        SuperDOM.setInnerHTML(e, str);
        SuperDOM.resizeImages(e, imgBoxSize);
        SuperDOM.addEvent(divId+'_GAL', 'click', function(e, event, target) {
          if (target.nodeName!='IMG')
           return;
          SuperDOM.showImgFullScreen(divId+'_popup', target.src)
        });
      }
  };
