...
define(["floria/superdom", "floria/controls", "floria/textutil", "floria/factories", "floria/date"]
     , function(SuperDOM, FloriaControls, FloriaText, FloriaFactories, FloriaDate) {
   ...
   if (form._formDefs.length > 1)
    {
      ...
      SuperDOM.addEvent(form._elementId+'_F_NEXT', "click", function() {
...
//            form._dlg.hide();
   ...
   if (form._cancelButton == true)
    SuperDOM.addEvent(form._elementId+'_F_CANCEL', "click", function() {
      ...
      if (d.keyUp == true)
        {
          ...
          SuperDOM.addEvent(f[fieldName], "keyup", function(e, event, target)
           {
             DelayedEvent.register(elementId+'_a_'+xid,500,function() {
               SuperDOM.fireEvent(f, "change");
             });
           });
        }
    }
...
var x = function(elementId, data, formDefs, edgeColumnCount, processCallbackFunc, popupTitle, wizardMode, liveOnChange, persist, validationErrMsg, cancelButton)
 { 
//   console.log("=>=>=> Forms2.constructor (data): ", data);
   ...
   if (SuperDOM.isObjectNullOrEmpty(data) == true && this._persistId != null)
    {
      data = Object.assign(data, SuperDOM.localStorageGet(this._persistId));
   ...
   this._data = SuperDOM.isObjectNullOrEmpty(data) == true ? { } : data;
   ...
   this.paintForm = function(pageId)
    {
          ...
          if (this._formDefs.length > 1)
           {
             ...
             var e = SuperDOM.getElement(this._elementId+'_F_PREV');
             if (e != null)
              e.disabled = pageId == 0;
             if (pageId == this._formDefs.length-1)
              {
                e = SuperDOM.getElement(this._elementId+'_F_NEXT');
                if (e != null)
                 e.disabled = true;
                if (this._popupTitle==null)
                 {
                   SuperDOM.hide(this._elementId+'_F_NEXT');
                   SuperDOM.show(this._elementId+'_F_SUBMIT');
                 }
              }
             else
              {
                ...
                SuperDOM.hide(this._elementId+'_F_SUBMIT');
              }
           }
        }
      ...
      if (this._collapseHTML != null)
       {
         var containerElement = SuperDOM.getElement(this._elementId);
         var containerNextElement = SuperDOM.nextSiblingNode(containerElement);
         var formElement = SuperDOM.getElement(this._elementId+"_F");
         SuperDOM.addEvent(formId+'_COLLAPSER', "click", function(e, event, target) {
           if (e.title == that._expandTitle) // need to expand
            {
              ...
              SuperDOM.fitBelow(containerElement, containerNextElement, 0);
            }
           else
            {
      ...
      if (this._liveOnChange == true)
        SuperDOM.addEvent(this._elementId+'_F', "change", function(e, event, target)
          {
...
//                                                             console.log("AFTER PICKER RENDER: _pickersLoading: "+that._pickersLoading, "; data: ", that._data);
                                              ...
                                              , this._liveOnChange == true ? function() {
                                                     SuperDOM.fireEvent(that._elementId+'_F', "change"); 
                                                  } : null
                                               );
        }
    }; 
...
//                   console.log("FORM VALUE --> d.name: "+d.name+"; f[d.name].value: "+f[d.name].value+"; typeof f[d.name].value: "+typeof f[d.name].value);
            ...
            if (this._persistId != null)
             SuperDOM.localStorageSet(this._persistId, this._data);
...
x.paintFieldList = function(elementId, fieldDefs, callbackFunc, editCallbackFunc)
  ...
  */
 x.fieldNotContains = function(data, value)
  {
    return data != null && data.length!=0 && data.indexOf(value)==-1;
  }
 x.fieldEmptyOrContains = function(data, value)
 {
   return data == null || data.length==0 || data.indexOf(value)!=-1;
 }
